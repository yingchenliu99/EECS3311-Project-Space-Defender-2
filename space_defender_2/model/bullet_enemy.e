note
	description: "Summary description for {BULLET_ENEMY}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
	...
	do
	  ...
	  loop
      	    ...
      	    across model.m.board.bullet_list is b_n loop
      	      ...
      	      then
                 enemy_bullet_collided(b_n)
      	      end
      	    end

      	    across model.m.board.bullet_list is b_n loop
      	      if
      	        b_n.row=current_row and b_n.column=current_column and attached {BULLET_SELF} b_n and b_n/=current and b_n.row>=1 and b_n.row<=model.m.board.board_row and b_n.column >=1 and b_n.column<=model.m.board.board_column
      	      then
      	      	 self_bullet_collided(b_n,current_row,current_column)
      	      end
      	    end
...
feature -- collided
  self_bullet_collided(enemy_b:BULLET;c_row:INTEGER;c_column:INTEGER)
  do
  	if
  ...
  enemy_bullet_collided(self_b:BULLET)
  do
  	self_b.disapper
  ...
  do
  	 ...
  	 message:=message+"healing "+bullet_damage.out+" damage."
     if
       enemy_b.current_health+bullet_damage<=enemy_b.health
     then
       enemy_b.set_current_health(enemy_b.current_health+bullet_damage)
     else
       enemy_b.set_current_health(enemy_b.health)
     end
  end
